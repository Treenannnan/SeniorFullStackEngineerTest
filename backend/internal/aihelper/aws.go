...
func (p *Pipeline) Run(callback func(PipelineState, *Output)) (*Output, error) {
	...
	origKey, clipKey := deriveS3KeysFromLocalPath(p.inputPath, p.keyAnchor, p.prefix)
...
Requirements:
- "summary": 200-250 characters, engaging/inviting, spark curiosity, no spoilers.
- Do NOT start with phrases like "This chapter", "This story", or similar.
- "matches": up to 5 items from existing categories with confidence in [0,1].
- "new_category": null OR { "label": "...", "confidence": 0.xx }.`},
	}
...
Produce exactly this JSON schema:
{
  ...
  "matches":[{"label":"<from existing>","confidence":0.xx}, ... up to 5],
  "new_category": null OR {"label":"<new>", "confidence":0.xx}
}
